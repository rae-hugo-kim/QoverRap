...
## 10. 출원 직전 12개월 재조사 (2026-04-30)

본 절은 출원 직전 시점에서 KIPRIS / EPO OPS / Google Patents 및 비특허 문헌에 대해 **공개일 2025-04-30 ~ 2026-04-30** 범위로 재조사한 결과이다. 이전 §3, §6, §7, §9 의 조사 시점(2026-04-17 ~ 2026-04-22) 이후 등재된 신규 문헌이 본 발명의 핵심 결합과 충돌하는지 확인하는 것이 목적이다.

### 10-1. KIPRIS 출원 직전 재조사

**조사 범위**: 공개일 또는 출원일 2025-04-01 ~ 2026-04-30, 12 개 키워드 조합 (한국어/영어), KIPRIS Plus OpenAPI `getAdvancedSearch`.

**검색 키워드**: QR 서명, QR 다층, QR 다중 레이어, QR 트레일러, QR 진본성, QR 검증 오프라인, Ed25519 QR, QR 검증, 이차원코드 서명, QR코드 인증, QR코드 서명, QR코드 다층, 영문 정밀 키워드 (QR delimiter / QR layered / QR signature trailer / QR payload).

**결과 요약**: 합산 818 건 (중복 제거), 도메인 매칭 후 90 건. 이 중 G06K 19/06 분류 51 건, wire-format 직접 관련 (레이어·다층 키워드 hit) 12 건. 영문 정밀 키워드 (Ed25519 QR / QR delimiter / QR layered / QR signature trailer / QR payload) 는 KR DB 에서 0 hit.

**신규 발견 — 주목 후보 3 건**:

| # | 출원번호 | 출원인 | 출원일 / 공개일 | IPC | 핵심 요지 | QoverwRap 차별점 |
|---|---------|--------|---------------|-----|---------|-----------------|
| 10-1-A | KR10-2024-0081386 (공개 KR10-2024-0081386A) | 주식회사 넥스팟솔루션 | 2024-06-21 / 2025-12-30 | G06K 19/06 | 복수 보안 요소를 구분한 복수 레이어 생성 + 순차 적층으로 식별코드 이미지 생성 (위조방지) | 이미지 레벨의 visual layering 으로 추정. QoverwRap 은 페이로드 문자열 레벨 delimiter-framed trailer 구조이며 QR 이미지 / ECC 미조작. 임베딩 위치·디코딩 방식·서명 포함 여부 모두 상이. 청구항 전문 확인 권고 |
| 10-1-B | KR10-2025-0093608 (등록) | 국립금오공과대학교 산학협력단 | 2025-07-11 / — | H04L 9/32, H04L 9/36, H04L 9/00 | JSON 문서를 QR 로 변환 + QR 에 검증자 서명정보 저장 + 블록체인 기반 시험평가서 진위판단 | 서명을 QR 에 포함하는 점은 유사하나 블록체인 서버 조회 의존 (QoverwRap: 온라인 서버 호출 없음). 단일 페이로드 구조 (delimiter-framed multi-layer wire format 부재). Layer A safe-fallback 부재. IPC 도 G06K 19/06 계열 아님 |
| 10-1-C | KR10-2025-7030169 (공개) | 쓰리엠 이노베이티브 프로퍼티즈 | 2024-02-14 (PCT/US 추정) / 2025-11-04 | G06F 21/62, H04W 4/80, H04L 9/40 | 스캔 가능한 식별자 구성 + 후속 최종 사용자 인증에 따른 편집 액세스 승인 (공유 가능 QR 웹앱) | 웹앱 서버 기반 접근 제어 + 외부 인증 메커니즘 — QoverwRap 의 offline self-contained · delimiter-framed signed payload 와 근본적으로 상이 |

**결론**: 90 건 중 3 건이 주목 후보로 확인되었으나, 모두 본 발명의 핵심 결합 (Layer A prefix + delimiter trailer + versioned header + safe-fallback) 을 가르치지 않는다. 다만 spec-draft.md §3.2 의 (나) / (라) 군에 대표 문헌으로 추가 표기 권장.

**산출물**: `docs/research/kipris-filtered-2025.json` (90 건), `docs/research/kipris-results-consolidated.json` (12 배치 원시 결과).

### 10-2. EPO OPS / Espacenet 출원 직전 재조사

**조사 범위**: `pd within "2025-04-30 2026-04-30"` 날짜 필터 적용 8 개 CQL 쿼리 + Google Patents / WIPO PATENTSCOPE 웹 보강.

**쿼리 결과**:

| Query ID | CQL 요약 | 결과 |
|----------|---------|------|
| QR-A | `cpc=G06K19/06 AND ti,ab=(signature AND QR)` + 날짜 필터 | **0** |
| QR-B | `ti,ab=(QR AND ed25519)` + 날짜 필터 | **0** |
| QR-C | `ti,ab=("layered QR" OR "multi-layer QR" OR "stacked QR" OR "multilayer QR")` + 날짜 필터 | **0** |
| QR-D | `ti,ab=(QR AND trailer AND signed)` + 날짜 필터 | **0** |
| QR-E | `ti,ab=(QR AND (delimiter OR "payload structur"))` + 날짜 필터 | **0** |
| QR-F | `ti,ab=("signed QR" OR "QR signature" OR "QR code signature")` + 날짜 필터 | **0** |
| QR-G | `ti,ab=(QR AND offline AND verification)` + 날짜 필터 | **0** |
| QR-H | `cpc=G06K19/06 AND ti,ab=(authenticity AND barcode)` + 날짜 필터 | **0** |
| 웹 보강 | Google Patents / PATENTSCOPE 보조 | 근접 0 건 (WO2024063800A1 TBCASoft 디지털 자격증명 / WO2024059884A1 RFID 서명 — 둘 다 무관 확인) |

**결론**: 조사 범위 내 신규 충돌 문헌 발견 없음. 기존 §3 ~ §9 의 결론 유지.

**한계 고지**: EPO OPS biblio DB 의 최근 출원 색인 지연 (공개일 기준 4~8 주 lag 가능) — 2026-03 이후 출원분의 일부가 미색인 상태일 수 있음. 출원 후 18 개월 시점 (2027 년 하반기) 의 추가 조사 권장.

**산출물**: `docs/research/epo-recheck-2026-04-30.json` (8 쿼리 응답 기록).

### 10-3. Google Patents / 비특허 문헌 출원 직전 재조사

**조사 범위**: Google Patents WebSearch 13 건 + WebFetch 보조 4 건 + 비특허 문헌 (arXiv / IEEE / ACM / IETF) WebSearch 6 건. 공개일 2025-04-30 ~ 2026-04-30. KIPRIS / EPO OPS 와의 중복 제거.

**신규 발견 — 8 건** (모두 본 발명과 무관 확인):

| 식별자 | 발행처 | 공개일 | 도메인 | QoverwRap 차별점 |
|--------|--------|--------|--------|-----------------|
| **US20250021779** | USPTO (Scott Wood 외) | 2025-01-16 | QR 스캔 후 URL 보안 위협 탐지 | 보안 분석기. Layer / delimiter 구조·서명·fallback 부재 |
| **US12328587B2** | USPTO (PCT/IB2021/061318) | 2025 등록 | Glyph payload 를 두 subset 으로 분할 + 외부 key-value DB 에 hash 키로 저장 | **서버 DB 조회 필수** (온라인 의존). 표준 스캐너 fallback 없음. delimiter trailer 부재 |
| **arXiv 2503.13458** "Dueling QR Codes" | arXiv | 2025-02-04 | 모듈 픽셀 반분할로 카메라 각도별 다른 메시지 | **모듈 비트(물리 픽셀) 조작** 방식. payload 구조화·서명·fallback 없음 |
| **PMC12252379 / Sensors 25(13) 3855** | MDPI Sensors | 2025-06-20 | LSB 워터마킹 + CNN 분류 기반 인증 | **LSB 픽셀 조작 + 신경망**. 일부 표준 스캐너 처리 불가 (Dynamsoft 의존) |
| **IJSRA-2025-0933** "Blockchain-based two-level QR" | IJSRA Vol 15(01) | 2025 | 블록체인 원장 + QR 암호화 | **블록체인 온라인 조회 필수**. 단일 페이로드 내 논리 레이어 구조 부재 |
| **ACM "Certifichain"** (10.1145/3754458) | ACM Digital Threats | 2025 | AES + ECDSA 자격증명을 QR 에 인코딩 + Hyperledger Fabric 연동 | **온라인 블록체인 검증 필수**. 표준 리더 fallback / delimiter 구조 / Layer A 평문 prefix 부재 |
| **IEEE 10877207** "Enhancing Printed Document Security" | IEEE Conf. | 2025 | 해시 + 비밀번호 암호화 → QR 인쇄 + 오프라인 해시 대조 | **비밀번호 재입력 필요**. 단일 payload (평문 prefix + signed trailer 분리 없음) |
| **IETF draft-steele-spice-cryptovolense-00** | IETF (individual, 만료) | 2024-01-02 (만료) | RaptorQ 기반 다중 프레임 QR 분할 | 애니메이션 다중 프레임 전송. 단일 정적 QR delimiter 구조·Layer A 평문·서명 알고리즘 미정의 |

**결론**: 발견된 8 건은 모두 (a) 모듈 / 픽셀 레벨 조작 (물리·광학·LSB), (b) 온라인 블록체인 / DB 조회 의존, (c) 비밀번호 기반 단순 해시 대조, (d) 다중 QR 프레임 전송 중 하나이며, QoverwRap 의 wire format 결합과의 직접 경합 요소가 없음.

### 10-4. §10 통합 결론

조사 범위 (KIPRIS + EPO OPS + Google Patents + 비특허) 내, 공개일 2025-04-30 ~ 2026-04-30 범위에서 본 발명의 핵심 5 요소 결합 — (i) Layer A 평문 prefix + (ii) delimiter-framed trailer + (iii) versioned binary header + (iv) 동일 페이로드 내 정규화된 서명 대상 + (v) 파싱·검증 실패 시 Layer A 만 반환하는 safe-fallback resolver — 을 동시에 가르치는 신규 문헌은 **발견되지 않았다**.

발견된 11 건 (KIPRIS 3 + Google Patents 8) 은 모두 spec-draft.md §3.2 의 (가) / (나) / (라) 군 분류에 대응하는 변형들이며, (마) 군 (서명-내장 자격증명/인증 QR) 또는 본 발명의 narrow combination 과 직접 경합하지 않는다.

**잔여 한계**:
- EPO OPS biblio DB 의 최근 출원 색인 지연 (4~8 주)
- 비특허 문헌의 cap 가 없는 광범위 검색 한계 (모든 학술지/conference proceedings 를 망라할 수 없음)
- 본 재조사는 키워드 기반 — 동의어 / 동등 표현으로 작성된 문헌이 누락될 가능성 잔존

위 한계는 모든 prior-art 조사 공통이며, 본 발명의 청구범위는 "narrow combination 의 wire-format 직접 결합" 에 집중되어 있어 동의어 누락에 강하다.
